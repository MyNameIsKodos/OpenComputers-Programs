--
--  Lua IDE
--  Made by GravityScore, optimized for OpenComputers
--
...
local theme = {
  ...
  button = 0x00FFFF,
  buttonHighlighted = 0xADD8E6,

  dangerButton =0xFF0000,
  dangerButtonHighlighted = 0xFFC0CB,
...
local function modRead(properties)
  ...
  local function sendLiveUpdates(event, ...)
    if type(properties.liveUpdates) == "function" then
      ...
      term.setCursor(ox, oy)
    end
  end
  
  term.setCursorBlink(true)
  while true do 
    local e, id, x, y = os.pullEvent()
    if e == "key_down" and x ~=0  then
      local char = unicode.char(x)
      local s = false
      if properties.textLength and line:len() < properties.textLength then
        s = true
      elseif not properties.textLength then
        s = true
      end
      ...
      if not properties.grantPrint and properties.refusePrint then
        ...
        for _, v in pairs(canTypeKeys) do 
          if char == v then 
            canType = false
          end
        end
      ...
      if s and canType then
        line = line:sub(1, pos) .. char .. line:sub(pos + 1, -1)
        pos = pos + 1
        redraw()
      end
    elseif y == "enter" then
    ...
    elseif (y == "up" or y == "down") and properties.history then
      redraw(" ")
      if y == "up" then
      ...
      elseif y == "down" then
        if historyPos == #properties.history then
          historyPos = nil
    ...
    elseif y == "home" then
      pos = 0
      redraw()
    elseif y == "delete" and pos < line:len() then
      redraw(" ")
    ...
    elseif properties.exitOnKey then
      if y == properties.exitOnKey or (properties.exitOnKey == "control" and (y == 29 or y == 157)) then
        term.setCursorBlink(false)
        return nil
      end
    end
    ...
    local a = sendLiveUpdates(e, id, x, y)
    if a then
      return a
    end
  end
...
local defaultTheme = {
  ...
  editorBackground = 0x808080,
  editorLineHightlight = 0xADD8E6,
  editorLineNumbers = 0x808080,
...
local function centerPrint(text, ny)
  if type(text) == "table" then for _, v in pairs(text) do centerPrint(v) end
    ...
    local w, h = gpu.getResolution()
    term.setCursor(w/2 - text:len()/2 + (#text % 2 == 0 and 1 or 0), ny or y)
    print(text)
  end
end
...
local function title(t)
  ...
  for i = 2, 4 do term.setCursor(1, i) term.clearLine() end
  term.setCursor(3, 3)
  term.write(t)
end
...
local function centerRead(wid, begt)
  ...
  gpu.setForeground(theme.textColor)
  gpu.setBackground(theme.promptHighlight)
  for i = 8, 10 do
    term.setCursor(w/2 - wid/2, i)
    term.write(string.rep(" ", wid))
  end

  if isAdvanced() then
    gpu.setBackground(theme.errHighlight)
    for i = 13, 15 do
      term.setCursor(w/2 - wid/2 + 1, i)
      term.write(string.rep(" ", 10))
    end
    term.setCursor(w/2 - wid/2 + 2, 14)
    term.write("> Cancel")
  end

  gpu.setBackground(theme.promptHighlight)
  term.setCursor(w/2 - wid/2 + 1, 9)
  term.write("> " .. begt)
...
local function prompt(list, dir, isGrid)
  local function draw(sel)
    for i, v in ipairs(list) do
      if i == sel then gpu.setBackground(v.highlight or theme.promptHighlight)
      else gpu.setBackground(v.bg or theme.prompt) end
      gpu.setForeground(v.tc or theme.textColor)
      for i = -1, 1 do
        term.setCursor(v[2], v[3] + i)
        term.write(string.rep(" ", v[1]:len() + 4))
      end

      term.setCursor(v[2], v[3])
      if i == sel then
        gpu.setBackground(v.highlight or theme.promptHighlight)
        term.write(" > ")
      else term.write(" - ") end
      term.write(v[1] .. " ")
    end
  ...
  while true do
    local e, id, x, y = os.pullEvent()
    if e == "key_down" and y == 28 then
      return list[sel][1]
    elseif e == "key_down" and y == key1 and sel > 1 then
      sel = sel - 1
      draw(sel)
    elseif e == "key_down" and y == key2 and ((err == true and sel < #list - 1) or (sel < #list)) then
      sel = sel + 1
      draw(sel)
    elseif isGrid and e == "key_down" and y == 203 and sel > 2 then
      sel = sel - 2
      draw(sel)
    elseif isGrid and e == "key_down" and y == 205 and sel < 3 then
      sel = sel + 2
      draw(sel)
    elseif e == "touch" then
      for i, v in ipairs(list) do
...
--  ["Settings"] = function() end,
  ...
  -- Functions
  ["Go To Line    ^+G"] = function() return nil, "go to", gotoline() end,
...
-- Run
  ...
  function waitForAny()
    ...
    local routines = { coroutine.create(function () main(arguments) end), coroutine.create(monitorKeyboardShortcuts) }
    return runUntilLimit( routines, #routines - 1 )
  end
    
    
    
-- TODO fix error catch
